...
export class DataIdentifierReassembler {
	...
	push(diFrameHex: string): string | null {
		if (this.pendingFrameRemainingBytes > 0) {
			...
			if (di != null && this.isNextDataIdentifierFrame(di)) {
				return this.pushNextDataIdentifierFrame(diFrameHex, di);
			}
	...
	private isNextDataIdentifierFrame(di: ParsedDataIdentifier): boolean {
		const expectedSeq = (this.frameCount + 1) & 0x0f;
		if (di.sequenceNumber != expectedSeq) return false;
		if (di.validBytes <= this.recvBuf.length) return false;
		if (di.validBytes > MAX_RECV_BUF_BYTES) return false;
		return true;
	}

	/**
	 * 上一段 payload 未达到 DI 声明的 vdn，但下一段又带了新的 DI 头。
	 * 文档和真机 0x3A 都可能是这种形态：每个 notify 都有 DI 头，vdn 表示累计 TLVC 长度。
	 * 此时应去掉新 DI 头，只追加新增 payload，而不是把 DI 头当作 TLVC 内容。
	 */
	private pushNextDataIdentifierFrame(
		diFrameHex: string,
		di: ParsedDataIdentifier
	): string | null {
		const expectedSeq = (this.frameCount + 1) & 0x0f;
		if (di.sequenceNumber != expectedSeq) {
			logger.warn("bluetooth",
		...
		this.frameCount = di.sequenceNumber;

		const availablePayloadHex = diFrameHex.substring(4);
		const targetBytes = Math.min(di.validBytes, MAX_RECV_BUF_BYTES);
		const needBytes = targetBytes - this.recvBuf.length;
		if (needBytes <= 0) {
			return this.finishLogicalFrame(di.isEnd);
		}

		const payloadHex = availablePayloadHex.substring(0, needBytes * 2);
		this.appendPayloadHex(payloadHex);
		this.pendingFrameRemainingBytes = Math.max(0, targetBytes - this.recvBuf.length);
		this.pendingFrameIsEnd = di.isEnd;
