// 蓝牙服务UUID映射表
export const SERVICE_UUID_NAMES: Record<string, string> = {
  // 常见蓝牙服务
...
// 蓝牙特征UUID映射表
export const CHARACTERISTIC_UUID_NAMES: Record<string, string> = {
  // Generic Access服务特征
...
export const getServiceName = (uuid: string): string => {
  return SERVICE_UUID_NAMES[uuid.toLowerCase()] || "Custom Service";
};

// 获取特征名称
export const getCharacteristicName = (uuid: string): string => {
  return (
    CHARACTERISTIC_UUID_NAMES[uuid.toLowerCase()] || "Custom Characteristic"
  );
};
